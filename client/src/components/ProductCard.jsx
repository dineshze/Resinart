    ...
    <article className="group overflow-hidden rounded-[28px] glass shadow-resin transition duration-300 hover:-translate-y-2">
      <Link to={`/product/${product._id}`} className="block">
        <div className="relative aspect-[4/3] overflow-hidden">
          <img src={cloudinaryImage(product.image, { width: 760 })} alt={product.name} loading="lazy" className="h-full w-full object-cover transition duration-700 group-hover:scale-110" />
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <Link to={`/product/${product._id}`} className="break-words font-display text-2xl font-bold">{product.name}</Link>
