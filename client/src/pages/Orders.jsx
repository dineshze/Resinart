  ...
  return (
    <main className="mx-auto max-w-6xl overflow-x-hidden px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-4">
        ...
        {!loading && orders.length === 0 && <EmptyState title="No orders yet" text="Placed orders and UPI payment status will appear here." />}
        {orders.map((order) => (
          <Link key={order._id} to={`/orders/${order._id}`} className="grid min-w-0 gap-4 rounded-[28px] glass p-5 shadow-resin transition hover:-translate-y-1 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-3">
                <PackageCheck className="text-lagoon dark:text-tide" />
                <h2 className="break-all font-display text-2xl font-bold">Order #{order._id.slice(-6).toUpperCase()}</h2>
            ...
            <div className="text-left md:text-right">
              ...
              <p className="text-sm capitalize text-ink/60 dark:text-pearl/64">{prettyStatus(order.paymentMethod || order.payment.method)} - {prettyStatus(order.paymentStatus || order.payment.status)}</p>
            </div>
          </Link>
        ))}
      </div>
